...
function distributeQuestions(words, questions, options = {}) {
  ...
  if (options.frequency) {
    ...
    const interval = Math.floor(sentenceEnds.length / totalQuestions);

    for (let i = 0; i < totalQuestions; i++) {
      // Add randomness within the interval
      const baseIndex = Math.min(i * interval, sentenceEnds.length - 1);
      const maxOffset = Math.min(interval - 1, sentenceEnds.length - baseIndex - 1);
      const randomOffset = Math.floor(Math.random() * (maxOffset + 1));
      const sentenceIndex = Math.min(baseIndex + randomOffset, sentenceEnds.length - 1);
